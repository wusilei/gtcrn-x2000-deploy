/**
 * gtcrn_fp.c — GTCRN MATLAB→C Fixed-Point Implementation
 * =======================================================
 * Verbatim MATLAB→C translation of GTCRN_speech_enhance_FPversion/.
 *
 * Q-format:
...
void ln_fixed(int32_t *x, int C, int Win,
    ...
    uint32_t var_q = (uint32_t)roundf(var_inv * 262144.0f);
...
void gru_fixed(const int32_t *x, int seq_len, int input_dim, int hidden_dim,
    ...
    for (int f = 0; f < seq_len; f++) {
        ...
        int16_t r_q15[8];
        for (int h = 0; h < hidden_dim; h++) r_q15[h] = F2Q15(sigmoidf_fp(Q10_TO_F(r_t[h])));
        ...
        int16_t z_q15[8];
        for (int h = 0; h < hidden_dim; h++) z_q15[h] = F2Q15(sigmoidf_fp(Q10_TO_F(z_t[h])));
        ...
        int16_t n_q15[8];
        for (int h = 0; h < hidden_dim; h++) n_q15[h] = F2Q15(tanhf_fp(Q10_TO_F(n_t[h])));
...
void mag_gen(const float *real_in, const float *imag_in, int N, int32_t *y) {
    for (int i = 0; i < N; i++) {
        float mag = sqrtf(real_in[i]*real_in[i] + imag_in[i]*imag_in[i] + 1e-12f);
        y[i] = F2Q20(mag);
        y[N+i] = F2Q20(real_in[i]);
        y[2*N+i] = F2Q20(imag_in[i]);
    }
}
...
void gtcrn_state_init(gtcrn_state_t *s) {
    memset(s, 0, sizeof(gtcrn_state_t));
}
...
void TRA_module(const int32_t *x, int16_t *h_prev, int gtc_idx,
    ...
    /* === Aggregation: square + mean across frequency → (8,) u32f20 === */
    int32_t x_agg[8];
    for (int c = 0; c < Cin; c++) {
        float sum = 0.0f;
        for (int w = 0; w < Win; w++) {
            float val = (float)x[c*Win+w] / 1048576.0f;
            sum += (float)(val * val);
        }
        x_agg[c] = (int32_t)roundf((float)(sum/Win) * 1048576.0f);
    }
    ...
    int16_t r_q15[16];
    for (int h = 0; h < hidden; h++) r_q15[h] = F2Q15(sigmoidf_fp(Q20_TO_F(r_t[h])));
    ...
    int16_t z_q15[16];
    for (int h = 0; h < hidden; h++) z_q15[h] = F2Q15(sigmoidf_fp(Q20_TO_F(z_t[h])));
    ...
    int16_t n_q15[16];
    for (int h = 0; h < hidden; h++) n_q15[h] = F2Q15(tanhf_fp(Q20_TO_F(n_t[h])));
    ...
    for (int co = 0; co < Cin; co++) {
        ...
        att[co] = F2Q15(sigmoidf_fp(Q20_TO_F(fc_out)));
    }
...
void DeTRA_module(const int32_t *x, int16_t *h_prev, int gtc_idx,
    ...
    /* === Aggregation: square + mean → (8,) u32f18 === */
    int32_t x_agg[8];
    for (int c = 0; c < Cin; c++) {
        float sum = 0.0f;
        for (int w = 0; w < Win; w++) {
        ...
        x_agg[c] = (int32_t)roundf((float)(sum/Win) * 262144.0f);
    }
    ...
    int16_t r_q15[16];
    for (int h = 0; h < hidden; h++) r_q15[h] = F2Q15(sigmoidf_fp(Q18_TO_F(r_t[h])));
    ...
    int16_t z_q15[16];
    for (int h = 0; h < hidden; h++) z_q15[h] = F2Q15(sigmoidf_fp(Q18_TO_F(z_t[h])));
    ...
    int16_t n_q15[16];
    for (int h = 0; h < hidden; h++) n_q15[h] = F2Q15(tanhf_fp(Q18_TO_F(n_t[h])));
...
void DeConv_block_0(const int32_t *x_in, const int32_t *x_skip,
    ...
    for (int i = 0; i < Cout*Wout; i++)
        y[i] = F2Q15(tanhf_fp(Q20_TO_F(y_conv[i])));
...
/* ================================================================
 * gtcrn_infer_frame — Single-frame full pipeline stub
 * ================================================================
 * The full wired version is in test_gtcrn_full.c.
 * This stub allows gtcrn_fp.c to compile without the weight header.
 */

/* Weak symbol — can be overridden by test_gtcrn_full.c which includes weights */
__attribute__((weak))
