/**
 * gtcrn_linux.c — GTCRN Linux 侧降噪主程序 (v15: +AGC +开关)
 * ============================================================
 * iccom rf11 读入 → GTCRN 降噪 → AGC 增益 → iccom wf12 写出
 *
 * 数据流: DSP 每 50ms 发 400×int16 @ 8kHz → rf11
 *         Linux: 能量采集 → GTCRN×2 → AGC → wf12
 *
...
int main(int argc, char **argv) {
    ...
    if (test_mode) {
         ...
         * ============================================================ */
        fprintf(stderr, "GTCRN v15 — test mode (8kHz stdin→GTCRN+AGC→8kHz stdout)\n");
        noise_init();
        agc_init();
        ...
        while (fread(in, sizeof(short), FRAME_IN, stdin) == FRAME_IN) {
            ...
            agc_process(out, FRAME_IN, energy);
         ...
         * ============================================================ */
        fprintf(stderr, "GTCRN v15 — iccom mode (rf11→GTCRN+AGC→wf12, 400smp/50ms)\n");
