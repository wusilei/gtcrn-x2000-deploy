/**
 * denoise_linux.c —  DENOISE  Linux 侧降噪主程序 (v20 — linux_api14, 前置AGC)
 * =====================================================================
 * 新管线 (3 阶段):
 ...
 *         -o denoise_linux_q15 agc.c denoise_linux.c noise_reduction_q15.c \
 *         denoise_fp.c denoise_infer.c -lm -lrt -static
 */
#include "agc.h"
#include <stdio.h>
#include <stdlib.h>
...
int main(int argc, char **argv) {
    ...
    if (measure_mode) {
        ...
        fprintf(stderr, "DENOISE v20 — iccom mode (rf11→AGC→NR→wf12, 400smp/50ms, state_flag=3)\n");
        int fd_rf = open("/dev/iccom_rf11", O_RDONLY);
        int fd_wf = open("/dev/iccom_wf12", O_WRONLY);
        if (fd_rf < 0 || fd_wf < 0) { perror("open"); return 1; }

        agc_init();
        fprintf(stderr, "DENOISE+AGC ready. noise=ON, agc=ON (state_flag=3, pre-AGC)\n");
