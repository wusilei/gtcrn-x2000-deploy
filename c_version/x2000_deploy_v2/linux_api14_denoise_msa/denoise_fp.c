...
void gru_fixed(const int32_t *x, int seq_len, int input_dim, int hidden_dim,
    ...
    for (int f = 0; f < seq_len; f++) {
        ...
        int16_t r_q15[8];
        for (int h = 0; h < hidden_dim; h++) r_q15[h] = sigmoid_q15(r_t[h]);  /* Q10→Q15 pure int */
        ...
        int16_t z_q15[8];
        for (int h = 0; h < hidden_dim; h++) z_q15[h] = sigmoid_q15(z_t[h]);  /* Q10→Q15 pure int */
        ...
        int16_t n_q15[8];
        for (int h = 0; h < hidden_dim; h++) n_q15[h] = tanh_q15(n_t[h]);  /* Q10→Q15 pure int */
...
void TRA_module(const int32_t *x, int16_t *h_prev, int gtc_idx,
    ...
    int16_t r_q15[16];
    for (int h = 0; h < hidden; h++) r_q15[h] = sigmoid_q15((r_t[h] + 512) >> 10);  /* Q20→Q10→Q15 pure int */
    ...
    int16_t z_q15[16];
    for (int h = 0; h < hidden; h++) z_q15[h] = sigmoid_q15((z_t[h] + 512) >> 10);  /* Q20→Q10→Q15 pure int */
    ...
    int16_t n_q15[16];
    for (int h = 0; h < hidden; h++) n_q15[h] = tanh_q15((n_t[h] + 512) >> 10);  /* Q20→Q10→Q15 pure int */
    ...
    for (int co = 0; co < Cin; co++) {
        ...
        att[co] = sigmoid_q15((fc_out + 512) >> 10);  /* Q20→Q10→Q15 pure int */
    }
...
void DeTRA_module(const int32_t *x, int16_t *h_prev, int gtc_idx,
    ...
    int16_t r_q15[16];
    for (int h = 0; h < hidden; h++) r_q15[h] = sigmoid_q15((r_t[h] + 128) >> 8);  /* Q18→Q10→Q15 pure int */
    ...
    int16_t z_q15[16];
    for (int h = 0; h < hidden; h++) z_q15[h] = sigmoid_q15((z_t[h] + 128) >> 8);  /* Q18→Q10→Q15 pure int */
    ...
    int16_t n_q15[16];
    for (int h = 0; h < hidden; h++) n_q15[h] = tanh_q15((n_t[h] + 128) >> 8);  /* Q18→Q10→Q15 pure int */
...
void DeConv_block_0(const int32_t *x_in, const int32_t *x_skip,
    ...
    for (int i = 0; i < Cout*Wout; i++)
        y[i] = tanh_q15((y_conv[i] + 512) >> 10);  /* Q20→Q10→Q15 pure int */
