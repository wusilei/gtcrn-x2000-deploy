/**
 * gtcrn_linux.c — GTCRN Linux 侧降噪主程序 (v21 — linux_api15, 混合AGC)
 * =====================================================================
 * 新管线 (3 阶段):
...
int main(int argc, char **argv) {
    ...
    if (measure_mode) {
        fprintf(stderr, "GTCRN v21 — measure mode (8kHz stdin→NR→8kHz stdout, no AGC)\n");
        agc_init();
        ...
        fprintf(stderr, "GTCRN v21 — test mode (8kHz stdin→NR→AGC→8kHz stdout)\n");
        agc_init();
        ...
        fprintf(stderr, "GTCRN v21 — BYPASS mode (rf11→wf12 passthrough)\n");
        ...
        fprintf(stderr, "GTCRN v21 — iccom mode (rf11→preAGC→NR→postAGC→wf12, 400smp/50ms, state_flag=3)\n");
        int fd_rf = open("/dev/iccom_rf11", O_RDONLY);
        int fd_wf = open("/dev/iccom_wf12", O_WRONLY);
        if (fd_rf < 0 || fd_wf < 0) { perror("open"); return 1; }

        agc_init();
        fprintf(stderr, "GTCRN+AGC ready. noise=ON, agc=ON (state_flag=3, hybrid-AGC)\n");
