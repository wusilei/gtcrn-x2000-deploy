...
/* sqrt-Hann window: sin(π*i/(N-1)) + pre-scaled /N_FFT */
static float sqrt_hann_win[WIN_LEN];
static float sqrt_hann_div_nfft[WIN_LEN];
static int   g_win_inited = 0;

/* Scale constants: multiply faster than divide on MIPS soft-float */
#define PCM_SCALE (1.0f / 32768.0f)
#define CRM_SCALE (1.0f / 1048576.0f)
...
void noise_init(void) {
    if (!g_win_inited) {
        for (int i = 0; i < WIN_LEN; i++) {
            ...
            sqrt_hann_div_nfft[i] = sqrt_hann_win[i] / (float)N_FFT;
        }
        g_win_inited = 1;
    }
...
void noise_reduction(short *voiceIn, short *voiceOut) {
    ...
    /* ── 2. STFT→GTCRN→ISTFT (时间正序: 最旧→最新) ── */
    {
        ...
        while (g_fifo_count >= WIN_LEN) {
            ...
            for (int i = 0; i < WIN_LEN; i++) ifft_out[i] = (float)ifft_q[i] * (1.0f/32768.0f);

            for (int i = 0; i < WIN_LEN; i++) {
                float sample = ifft_out[i] * sqrt_hann_div_nfft[i];
