# GTCRN 纯 C 定点 — Linux 侧降噪集成指南 (v12, 50ms iccom, 定点性能优化)
...
**版本**: v12 (2026-06-24)
**状态**: ✅ X2000 验证通过, 全部 bit-exact with PC, MIPS 软浮点性能大幅优化
**与 v11 (linux_api4) 差异**: Sigmoid/Tanh LUT + 定点 LayerNorm + 整数幅值，接口不变，降噪效果一致，性能显著提升
...
| GTCRN 推理 | 纯 C 定点, LUT 激活 + 定点 LayerNorm (显著快于 v11, 待 X2000 实测) |
| 内存 | ~422KB (state 70KB + 权重 302KB + LUT 2KB + 缓冲 50KB) |
...
```
linux_api5/
├── gtcrn_linux.c              ← 主程序 (iccom 50ms / test / test16k)
├── noise_reduction.h          公开 API (与 linux_api4 相同)
├── noise_reduction.c          流式降噪适配层 (v12: 预缩放窗表, 乘代除)
├── noise_reduction_id.c       直通模式变体 (passthrough, 调试用)
├── noise_reduction_q15inv.c   Q15 FFT 变体 (定点 FFT 优化, 引用 linux_api6)
├── gtcrn_fp.h                 GTCRN 定点推理头文件 (v12: LUT Sigmoid/Tanh)
├── gtcrn_fp.c                 GTCRN 定点推理实现 (v12: 定点 LayerNorm + 整数幅值)
├── gtcrn_infer.c              推理管线 (与 linux_api4 相同)
├── gtcrn_matlab_weights.h     定点权重 (与 linux_api4 相同)
├── kiss_fft.c/h               KissFFT 复数 FFT (与 linux_api4 相同)
├── kiss_fftr.c/h              KissFFT 实数 FFT (与 linux_api4 相同)
├── gtcrn_linux                PC 可执行文件 (x86)
├── gtcrn_linux_mips           X2000 MIPS32R2 可执行文件 (868KB, 比 v11 缩小 12KB)
├── gtcrn_linux_lut            LUT 版本 PC 可执行文件
├── gtcrn_linux_test           测试版本 PC 可执行文件
...
| 验证项 | 结果 |
|:--|:--|
| 5 文件 MATLAB 测试 | 全部 bit-exact (X2000 vs PC vs linux_api4) ✅ |
| 8 文件对讲机实录 | 全部 bit-exact (X2000 vs PC vs linux_api4) ✅ |
| test/test16k 模式 | 与 linux_api4 完全一致 ✅ |
| 滋滋噪声 | ✅ 已消除 |
| 人声可懂度 | ✅ 正常 |
| iccom 50ms 适配 | ✅ 接口不变 |
| LUT 精度 | ✅ 误差 <1 LSB Q15 (lossless vs expf) |
| X2000 性能 | 显著快于 v11 (待实测) |

---

## v11→v12 核心变更

v12 针对 MIPS 软浮点的性能优化，接口、权重、降噪效果与 v11 完全一致。

| 变更 | v11 (linux_api4) | v12 (linux_api5) |
|:--|:--|:--|
| Sigmoid/Tanh | soft-float expf/tanhf (~1000 cycles) | 512-entry LUT + 线性插值 (~30 cycles) |
| LayerNorm | float mean/var + sqrtf + float div | int64 累加 + isqrt64 + int reciprocal |
| 幅值计算 | sqrtf(real²+imag²) | isqrt64 (整数 sqrt) |
| 窗缩放 | 运行时 /N_FFT | 预计算 sqrt_hann_div_nfft 表 |
| PCM/CRM 缩放 | /32768.0f, /1048576.0f | ×PCM_SCALE, ×CRM_SCALE (乘代除) |
| MIPS 二进制 | 880KB | 868KB (移除 soft-float 库代码) |

## v10→v11 差异 (gtcrn_linux.c, 仅供参考)
