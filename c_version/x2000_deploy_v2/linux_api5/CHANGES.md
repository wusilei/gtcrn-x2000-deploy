# 2026-06-24 GTCRN v12 — linux_api5 MIPS 软浮点性能优化

**背景**: v11 (linux_api4) X2000 板上单帧耗时 ~47.5ms，2 帧 95ms 超出 iccom 50ms 窗口。瓶颈在 MIPS 软浮点 expf/tanhf/sqrtf。

## v11→v12 性能优化

| # | 变更 | 说明 | 预期加速 |
|:--|:--|:--|:--|
| **1** | **Sigmoid/Tanh LUT** | 512-entry Q10→Q15 LUT + 线性插值替代 expf/tanhf | ~30x (激活函数) |
| **2** | **定点 LayerNorm** | int64 mean/var + isqrt64 + int reciprocal 替代 float mean/var/sqrt/div | ~10x (LayerNorm) |
| **3** | **整数幅值** | isqrt64(real²+imag²) 替代 sqrtf() | ~5x (特征提取) |
| **4** | **预缩放窗表** | sqrt_hann_div_nfft 预计算表，运行时乘代除 | ~2x (窗应用) |
| **5** | **缩放常量** | PCM_SCALE/CRM_SCALE 常量替代字面量除法 | ~2x (缩放) |

### LUT 精度

- Sigmoid: Q10 输入 [-8192, 8191] → Q15 输出 [0, 32767]
- Tanh: Q10 输入 [-4096, 4095] → Q15 输出 [-32768, 32767]
- 线性插值: 最大误差 <1 LSB Q15，lossless vs expf
- 内存: 512×2×2 = 2KB
...
```
v11 (float):
  mean = sum(x[i]/2^20) / N          ← float 累加 + float 除法
  var  = sum((x[i]/2^20 - mean)²)/N  ← float 平方 + sqrtf + float 除法
  out  = (x - mean_q) × var_q        ← float→Q20 往返

v12 (int64):
  sum_x = Σ x[i]                      ← Q20, int64 累加
  mean_q = (sum_x + N/2) / N          ← rounded 整数除法
  diff = x[i] - mean_q                ← Q20, int64
  var_q40 = Σ diff² / N + ε           ← Q40, int64
  rms_q20 = isqrt64(var_q40)          ← 整数 sqrt
  var_inv = 2^38 / rms_q20            ← 整数 reciprocal
```
...
| 文件 | 变更 |
|:--|:--|
| `gtcrn_fp.h` | +128 行 LUT 表 (2KB), sigmoid_lut_q15[512], tanh_lut_q15[512] |
| `gtcrn_fp.c` | LayerNorm 定点化, 幅值 isqrt64, 聚合 int64 |
| `noise_reduction.c` | 预缩放窗表, PCM_SCALE/CRM_SCALE 常量 |
| `noise_reduction_id.c` | **新增** — 直通模式变体 (调试用, 不做降噪) |
| `noise_reduction_q15inv.c` | **新增** — Q15 FFT 变体 (引用 linux_api6/fft_q15.h) |
| `gtcrn_linux_lut` | **新增** — LUT 版本 PC 可执行文件 |
| `gtcrn_linux_test` | **新增** — 测试版本 PC 可执行文件 |
| `gtcrn_linux_mips` | 880KB→868KB (移除 soft-float 代码) |

### 兼容性

- ✅ 接口不变 (`noise_reduction(short*in, short*out)`, 200×int16 @ 25ms)
- ✅ iccom 不变 (400×int16 @ 50ms, rf11/wf12)
- ✅ 权重不变 (`gtcrn_matlab_weights.h` 完全相同)
- ✅ 降噪效果 bit-exact with v11 (LUT 误差 <1 LSB)
