# GTCRN Linux 侧 — DSP 联调指南 (v12, 50ms iccom, 定点性能优化)

## 编译好的二进制

```
x2000_deploy_v2/linux_api5/gtcrn_linux_mips  (868KB, MIPS32R2, 静态链接, 定点性能优化)
```
...
```sh
scp linux_api5/gtcrn_linux_mips root@192.168.42.159:/usr/bin/gtcrn_linux
...
- v12 定点性能优化 (Sigmoid/Tanh LUT 替代 soft-float expf/tanhf, 定点 LayerNorm, 整数幅值计算, 乘代除)
- 单帧 `noise_reduction` 预计显著快于 v11 (待 X2000 实测)
- iccom 50ms 周期内需完成 **2 帧**
- **P1 优化目标**: 每帧 <25ms，使 2 帧总耗时 <50ms
- 优化方向: FFT 定点化 (Q15 FFT, 参见 `noise_reduction_q15inv.c`)
...
| `linux_api4/` | v11 final | 400/50ms | ❄️ 冻结 |
| `linux_api5/` | **v12** | **400/50ms** | ✅ 当前交付 (定点性能优化) |

> **v11→v12 (DSP 侧无影响)**: iccom 接口不变 (400×int16 @ 50ms, rf11/wf12)。
> 仅 Linux 侧内部性能优化 (LUT 激活函数 + 定点 LayerNorm + 整数幅值)，降噪效果 bit-exact 一致，二进制体积缩小 (880KB→868KB)。
